...
def get_time_completo(time_id, rodada, is_live):
    """Necessário para pegar o capitão_id"""
...
def main():
    ...
    if not is_live:
        ...
        print(f"🚀 Nova rodada fechada detectada! Iniciando carga Oficial...")
    else:
        print(f"⚡ Atualizando parciais AO VIVO...")
    ...
    times_liga = res_liga.json().get('times', []) if res_liga.status_code == 200 else []
    
    if not times_liga:
        print(f"❌ Erro ao buscar times da liga {LIGA_SLUG}. Status: {res_liga.status_code}")
        return

    ts_agora = datetime.now(pytz.timezone('America/Sao_Paulo'))
    l_hist, l_esc, l_atl = [], [], []

    # A) Processa Tabela de Atletas Globais
